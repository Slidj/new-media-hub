...
import { Star, Tv, Clock, Bookmark, ListVideo } from 'lucide-react';
...
export const MyListView: React.FC<MyListViewProps> = ({ myList, history, onMovieSelect, lang }) => {
  ...
  return (
    <div className="fixed inset-0 z-30 w-full bg-black flex flex-col">
      
      {/* Sticky Header & Tabs */}
      <div className="sticky top-0 z-40 w-full bg-black pt-[calc(env(safe-area-inset-top)+140px)] pb-4 px-4 md:px-12">
          <div className="flex items-center gap-3 mb-6">
              <div className="p-2 rounded-full shadow-lg bg-[#E50914] shadow-red-900/20">
                <ListVideo className="w-5 h-5 text-white" />
              </div>
              <h2 className="text-2xl font-bold text-white">{t.myList}</h2>
          </div>
          
          <div className="flex p-1 rounded-lg w-full max-w-sm bg-[#141414] border border-white/5">
             <button 
      ...
      {/* Scrollable Content */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain no-scrollbar pt-6 pb-32 md:pb-24">
          {isEmpty ? (
